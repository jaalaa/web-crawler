...
// price
// https://www.eichholtz.com/en/dynamicpricingstock/price/get?sku=113801
...
async function login_eichholtz( _json_array){
	...
	if( _json_array.length > 0){
		...
		try{
			...
			client.fetch(eichholtz_getListURL("login"), function (err, $, res, body) {
				...
				// 	// console.log( $('div.page-title-wrapper').text()); // => bar
				// });

				var result2 = form.find('button[id=send2]').clickSync();

				console.log( result2.$('div.page-title-wrapper').text()); // => bar
			...
			// 	console.log(ret.$('form').body)
			// 	console.log(config.user_info.username)
			// 	console.log(config.user_info.password)

				// login
				// var ret2 = ret.$('form[id=login-form]').submitSync({
				// var ret2 = ret.$('form').submitSync({
				// 	'login[username]': config.user_info.username,
				// 	'login[password]': config.user_info.password,
				// });
...
function updateJSON( _i, _obj, _detail){
	...
	var url = 'https://www.eichholtz.com/en/dynamicpricingstock/price/get?sku=' + _obj['品番'];

	var price = client.fetchSync( url )
	console.log(url)
