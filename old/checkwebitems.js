...
async function main(){
	...
	client.fetch(
		...
		// login
		var ret2 = result.$('form[id=login-form]').submitSync({
		...
		if (ret2.error || !ret2.response || ret2.response.statusCode !== 200) {
			...
			writelog("[" + yyyymmdd_hhmm() + "]" + type +"Page Title(" + ret2.$('div.page-title-wrapper').text() + ")", true);
	...
	do {
		...
		try{
			...
			if( errorCount > 5){ break}
				...
				setTimeout(() => { 
					writelog("[" + yyyymmdd_hhmm() + "]" + type +"Wait time 2s",true);
				}, 2000);
				errorCount++;
			}
		}
