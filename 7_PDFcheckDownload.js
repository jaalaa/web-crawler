...
async function download_one_item( _item, _i,  errorback, toatalNumber ){

    // checkDir( _item.eichholtz_id, function(){

        _i++

        console.log("ID: " + _item.eichholtz_id + " (" + _i + '/' + toatalNumber + ")")
...
async function readItemsInfo4DB( _dataArray ){
    ...
    // get URL
    let downloadInfo = await db.eichholtz.find( { $and:[ {eichholtz_id: { $in : download_list }}, {date: targetDate } ] } ,{ date:1, eichholtz_id : 1, eichholtz_url: 1 , _id: 0 });
...
function checkDownload(){
    // target download
    fs.readdir(downloadDirPDF, async function(err, files){
        if (err) throw err;
        var fileList = files.filter(async function( file){
            var state = fs.statSync(downloadDirPDF + file)
            // console.log(state.size)
            if(state.size > 200000){
                await db.tmp.update({name:file}, {$set :{size:state.size, download:'OK'}})
                
                if(state.size < 500000){
                    console.log('open and check: ' + file)
                }
                return state.isFile() && /.*\.pdf$/.test(downloadDirPDF + file); //絞り込み
            }
        })

        let finalCheck = await db.tmp.find({$and:[{ $not: { id:''} },{$not: { download:'OK'} }]})
        ...
        if( finalCheck.length == 0){
            ...
            for( let i=0; i<downloadItems.length;i++){
                ...
                await setTimeout(download_one_item, Math.round( i ) * config.downloadInfo.time * 1000, downloadItems[i], i, null , downloadItems.length , errorback);
...
async function main(){
    ...
    var listTarget = readTargetID();

    for(let i = 0; i < listTarget.length; i++){
        // console.log(listTarget[i])
        await db.tmp.insert(
            {
                id: listTarget[i],
                name: listTarget[i] + '.pdf',
            }
        );    
    }
    checkDownload()
